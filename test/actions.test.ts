...
describe("planJump", () => {
  test("branch row → switch", () => {
    expect(planJump(row((r) => r.kind === "branch" && r.sessionID === SQUASHED), view, { currentSessionID: TRUNK })).toEqual({ kind: "switch", sessionID: SQUASHED })
  })
  test("middle user turn → fork at that message with prefill (redo)", () => {
    const t2 = row((r) => r.kind === "turn" && r.messageID === "m2")
    expect(planJump(t2, view, { currentSessionID: TRUNK })).toEqual({ kind: "fork", sessionID: TRUNK, messageID: "m2", prefill: "decompress the session and inspect it", mode: "redo" })
  })
  test("step row → fork at the next message (continue)", () => {
    const step = row((r) => r.kind === "step" && r.messageID === "a1")
    expect(planJump(step, view, { currentSessionID: TRUNK })).toEqual({ kind: "fork", sessionID: TRUNK, messageID: "m2", mode: "continue" })
  })
  test("tip of the current session → noop; tip of another session → switch", () => {
    expect(planJump(view.rows[view.indexById[view.currentRowId!]!]!, view, { currentSessionID: TRUNK }).kind).toBe("noop")
    const openTip = [...view.rows].reverse().find((r) => r.kind !== "branch" && r.sessionID === OPEN)!
    expect(planJump(openTip, view, { currentSessionID: TRUNK })).toEqual({ kind: "switch", sessionID: OPEN })
  })
  test("from inside a branch, a prefix row forks the trunk, not the copy", () => {
    ...
    expect(planJump(t1, inner, { currentSessionID: OPEN })).toMatchObject({ kind: "fork", sessionID: TRUNK, messageID: "m1", mode: "redo" })
  })
})
